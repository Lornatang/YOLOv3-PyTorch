...
def detect(save_image=False):
    ...
    if weights.endswith(".pth"):
        model.load_torch_weights(torch.load(weights, map_location=device)["state_dict"])
    else:
    ...
    if classify:
        ...
        # load model
        model_classifier.load_torch_weights(torch.load("weights/resnet101.pth", map_location=device))
